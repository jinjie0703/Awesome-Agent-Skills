...
def replace_text_in_docx(file_path: str, old_text: str, new_text: str, output_path: str = None):
    ...
    for p in doc.paragraphs:
        if old_text in p.text:
            if replace_in_runs(p.runs, old_text, new_text):
                count += 1
    ...
    doc.save(save_path)
    return f"成功在文档中完成了 {count} 处段落/单元格的精准无损替换: '{old_text}' -> '{new_text}'，并保存至: {save_path}（原文件已备份至: {bak}）"
...
if __name__ == "__main__":
    ...
    msg = replace_text_in_docx(args.file_path, args.old, args.new, args.output)
    print(msg)
