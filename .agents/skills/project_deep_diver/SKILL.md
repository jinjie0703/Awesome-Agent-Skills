---
name: project_deep_diver
description: 秋招/社招项目深挖面经准备技能。自动扫描项目代码指纹，提炼 STAR 法则亮点，预测面试官连环拷问并生成高分防御手册，支持模拟面试官实时对线。在需要复盘项目亮点、准备大厂面试项目深挖或生成面经手册时使用。
category: career-preparation
tags: [interview, resume, project, STAR, mock-interview]
---
...
- 核心依赖清单（自动解析 `package.json` / `requirements.txt` / `pom.xml` / `go.mod`）
- 顶层目录结构大纲
...
- **难点深挖**：如果用户觉得项目"太简单"，从**高并发、大数据量、极端边界、性能瓶颈、内存优化、网络异常、缓存穿透/雪崩、RAG 召回精度**等工程维度，为其构思和挖掘 1~3 个真实可信的架构难点。
...
4. **性能极限关**：你在项目中做了哪些关键优化？核心指标（如首屏时间、内存占用、QPS、TTFT）提升了多少？
...
如果用户要求“考考我”、“来模拟面试”，AI 立即切换为**严厉且专业的资深技术面试官**：
1. **单题单问**：一次只提出 1 个 5 连环拷问层级的问题，绝不一次性发一大堆问题；
2. **答复评分与点评**：用户回答后，AI 给出评分（1~10 分），指出回答中的薄弱点（如缺乏量化指标、原理不深、方案不够健壮）；
3. **高分模范话术示范**：示范如何以资深工程师的口吻优雅回答，随后推进到下一轮追问。
